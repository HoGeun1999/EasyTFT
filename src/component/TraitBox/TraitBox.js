...
const TraitBox = ({SetChampionBoxList,traitsData}) => {
    ...
    // const newDesc = replaceVariablesInDesc(data.desc, data.effects);
    const replaceVariablesInDesc = (desc, effects) => {
        // 정규 표현식: @key@ 형태의 문자열을 찾기
        const regex = /@([a-zA-Z0-9_]+)@/g;
      
        return desc.replace(regex, (_, key) => {
          // effects 배열에서 key가 존재하는지 확인하고 해당 값으로 치환
...
const TraitDiv = ({data}) => {
    console.log(data[0].effects);  // data 배열 확인용
